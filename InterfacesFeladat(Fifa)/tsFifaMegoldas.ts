...
function fifaObjektum(elemek: string[]): FifaAdat[] {
    var beolvasottAdatok: FifaAdat[] = [];
    for (let i: number = 0; i < elemek.length; i++) {
        let daraboltSor: string[] = elemek[i].split(";");
...
var fifaAdatok: FifaAdat[] = fifaObjektum(csapatAdat);

function adatokKiirasa(adatok: FifaAdat[]): void {
...
function csapotokSzama(adatok: FifaAdat[]): void {
...
function atlagPontszam(adatok: FifaAdat[]): number {
    let pontSzam: number = 0;
    for (let i: number = 0; i < adatok.length; i++) {
...
function atlagnalTobbPont(adatok: FifaAdat[]): void {
    for (let i: number = 0; i < adatok.length; i++) {
...
function legtobbetJavito(adatok: FifaAdat[]): void {
    let javitoIndex: number = 0;
    for (let i: number = 0; i < adatok.length; i++) {
...
function magyarorszagBentVanE(adatok: FifaAdat[]): boolean {
...
function eredmenyKiiro(eredmeny: boolean, igazUzenet: string, hamisUzenet: string): void {
    if (eredmeny == true) {
